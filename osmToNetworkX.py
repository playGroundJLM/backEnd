...
class OSM:
    def __init__(self, filename_or_stream):
        ...
        copyWays = copy.copy(self.ways)
        for way in copyWays.values():
...
if __name__ == "__main__":
    op = str(sys.argv[1]).lower()
    path = str(sys.argv[2])
    if op == "write":
        print("[Graph] osmToNetworkX: write")
        g = read_osm("./Jerusalem.osm")
        networkx.write_gpickle(g, str(path))
    elif op == "read":
        print("[Graph] osmToNetworkX: read")
        g = networkx.read_gpickle(path)
    else:
        print("usage: " + __file__ + " <read/write> <path>")
        exit(0)
    ...
    for i in cycles:
        if len(i) in range(20, 100):
            for j in i:
                n = g.node[j]
                print("{0}, {1}".format(n["lat"], n["lon"]))
            break
